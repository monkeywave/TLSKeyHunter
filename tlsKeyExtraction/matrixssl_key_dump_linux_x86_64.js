...
frida -p $(frida-ps | grep -i matrix | awk '{print $1}') -l universal_key_dump_liunx_x86_64.js

*/

var CLIENT_TRAFFIC_SECRET_0 = "";
var SERVER_TRAFFIC_SECRET_0 = "";
var CLIENT_HANDSHAKE_TRAFFIC_SECRET = "";
var SERVER_HANDSHAKE_TRAFFIC_SECRET = "";
var GLOBAL_CLIENT_RANDOM = "";
var keep_dumping = true;
var first_iv_done = false;
...
// Function to parse the SSL_HANDSHAKE structure of MatrixSSL
    ...
    try{
        if (!ptr.isNull()) {
            ...
            if(labelStr === null){
                ...
                // || labelStr.includes("master secret") --> then the out value is the key
...
function get_client_random_of_arg10(client_random_ptr){
    var RANDOM_KEY_LENGTH = 32;
    const keyData = Memory.readByteArray(client_random_ptr.add(0x10), RANDOM_KEY_LENGTH); // Read the key data (KEY_LENGTH bytes)
...
function dump_keys_from_derive_secrets(client_random_ptr, label_to_span,key, key_len) {
    var KEY_LENGTH = 0;
    // Set the expected length of the key (in bytes). Adjust as needed.
    if(key_len < 30){
    ...
    if (!label_to_span.isNull()) {
        ...
        label = label_to_span.readCString(); // Read the C string --> hier scheint es unter x86-64 eine access violation zu geben
        //console.log("label_to_span: "+label);
        labelStr = getTLSLabel(label);
        if(labelStr === null){
            if(label.includes("iv")){
                try{
                GLOBAL_CLIENT_RANDOM = get_client_random_of_arg10(client_random_ptr);
                if(first_iv_done == false){
                    first_iv_done = true;
                    return false;
                }
                return true;
    ...
    if (!key.isNull()) {
    try{
        var keyData = Memory.readByteArray(key, KEY_LENGTH); // Read the key data (KEY_LENGTH bytes)
    ...
    if(labelStr.includes("CLIENT_TRAFFIC_SECRET_0")){
        CLIENT_TRAFFIC_SECRET_0 = secret_key;
    }else if(labelStr.includes("SERVER_TRAFFIC_SECRET_0")){
        SERVER_TRAFFIC_SECRET_0 = secret_key;
    }else if(labelStr.includes("CLIENT_HANDSHAKE_TRAFFIC_SECRET")){
        CLIENT_HANDSHAKE_TRAFFIC_SECRET = secret_key;
    }else if(labelStr.includes("SERVER_HANDSHAKE_TRAFFIC_SECRET")){
        SERVER_HANDSHAKE_TRAFFIC_SECRET = secret_key;
    }
...
function dump_keys_from_prf(client_random_ptr, key, key_len) {
    ...
    if(!client_random_ptr.isNull()){
        ...
        Memcpy(msSeed, LABEL_EXT_MATERSEC, LABEL_EXT_SIZE);
        Memcpy(msSee + LABEL_EXT_SIZE, hash, outlen);
        */
        const keyData = Memory.readByteArray(client_random_ptr.add(0x2D), CLIENT_RANDOM_LENGTH); // Read the key data (KEY_LENGTH bytes)
...
function hookMatrixSSLByPattern(module) {
    ...
    var pattern_mb_derive_secret = "F3 0F 1E FA 41 57 4D 89 C7 41 56 41 55 41 54 49 89 FC 55 53 44 89 CB 48 81 EC 08 01 00 00 48 8B 84 24 40 01 00 00 44 8B AC 24 50 01 00 00 89 74 24 18 48 8D 6C 24 40 44 8B B4 24 48 01 00 00 48 89 54 24 10 48 89 EE 48 89 44 24 28 48 8B 84 24 58 01 00 00 89 4C 24 1C 43 8D 54 0E 0A 44 89 6C 24 04 0F B7 D2 48 89 44 24 20 64 48 8B 04 25 28 00 00 00 48 89 84 24 F8 00 00 00 31 C0 E8 EE 22 04 00";
    //console.log("[*] Using HKDF pattern: "+pattern_mb_derive_secret)

    // Try first pattern and if it fails, try the second one
    hook_HKDF_By_Pattern(moduleBase, moduleSize, pattern_mb_derive_secret, "psHkdfExpandLabel");
    //var pattern_prf = "F3 0F 1E FA 41 57 45 89 CA 41 56 41 55 41 54 55 89 CD 53 89 F3 48 81 EC 48 03 00 00 48 89 7C 24 38 48 89 54 24 08 4C 89 44 24 48 66 44 89 4C 24 50 64 48 8B 04 25 28 00 00 00 48 89 84 24 38 03 00 00 31 C0 66 41 81 F9 E0 00 0F 87 10 02 00 00"; // this is only for TLS 1.0 and TLS 1.1
    var pattern_prf = "F3 0F 1E FA 41 57 45 89 CA 41 56 41 55 49 89 FD 41 54 41 89 F4 55 53 89 CB 48 81 EC 38 03 00 00 48 89 54 24 08 4C 89 44 24 48 44 89 4C 24 34 64 48 8B 04 25 28 00 00 00 48 89 84 24 28 03 00 00";
                        ...
                        Interceptor.attach(address, {
                            ...
                            onLeave: function(retval) {
                                ...
                                //if (!retval.isNull()) {
                                    ...
                                    // client_random, label_to_span,key, key_len)
                                    //console.log("this.key: "+this.key);
                                    //console.log("this.myargs[i]: "+this.myargs[9]);
                                    if(keep_dumping){
                                    var ret = dump_keys_from_derive_secrets(this.client_random_ptr,this.label,this.key,this.key_len);
                                    //if(ret && already_done){
                                    if(ret){
                                        console.log("\n\ndumping keys: ")
                                        console.log("CLIENT_HANDSHAKE_TRAFFIC_SECRET "+GLOBAL_CLIENT_RANDOM+" "+CLIENT_HANDSHAKE_TRAFFIC_SECRET);
                                        console.log("SERVER_HANDSHAKE_TRAFFIC_SECRET "+GLOBAL_CLIENT_RANDOM+" "+SERVER_HANDSHAKE_TRAFFIC_SECRET);
                                        console.log("CLIENT_TRAFFIC_SECRET_0 "+GLOBAL_CLIENT_RANDOM+" "+CLIENT_TRAFFIC_SECRET_0);
                                        console.log("SERVER_TRAFFIC_SECRET_0 "+GLOBAL_CLIENT_RANDOM+" "+SERVER_TRAFFIC_SECRET_0);
                                        console.log("\n");
                                        keep_dumping = false;
                                        //already_done = false;
                                    }
                    ...
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                ...
                                //this.label = 
                                this.key = args[4];
                                this.key_len = args[5].toInt32();
                                this.seed = args[2]; // this also contains the label
                                ...
                                */
                                ...
                                if(is_arg_key_exp(args[2])){
                                    //this.client_random_ptr = args[8];
                                    /*
                                    although the key is generated/written in arg[4]
                                    the PRF is also used for the key expansion and used the master secret 
                                    as its input secret therfore we use that in order to get client_random 
                                    */
                                    this.key = args[0];
                            ...
                            onLeave: function(retval) {
                                ...
                                if (!retval.isNull() && this.dump_keys) {
                                    dump_keys_from_prf(this.seed, this.key, this.key_len);
...
// Find the MatrixSSL module
function findMatrixSSLModule() {
    var modules = Process.enumerateModules();
    for (var i = 0; i < modules.length; i++) {
        var name = modules[i].name;
        if (name.startsWith("test_client_") ) {
            console.log("Found MatrixSSL Module: " + name);
            return modules[i];
        }
    }
    console.log("MatrixSSL module not found.");
    return null;
}
...
// Main function
function main() {
    var module = findMatrixSSLModule();
    if (module !== null) {
        hookMatrixSSLByPattern(module);
    }
}
...
[*] HKDF-Function identified with label: PSHKDFEXPANDLABEL (psHkdfExpandLabel)
[*] HKDF-Function signature: undefined psHkdfExpandLabel(undefined param_1, undefined param_2, undefined param_3, undefined param_4, undefined param_5, undefined param_6, undefined8 param_7, undefined4 param_8, undefined4 param_9, undefined8 param_10)
[*] String (Register R8) used as 4th argument in function call at address: 0x00426493 (invoking PSHKDFEXPANDLABEL)
[*] Function offset (Ghidra): 00443D70 (0x00443D70)
[*] Function offset (IDA with base 0x0): 00343D70 (0x00343D70)
[*] Byte pattern for frida: F3 0F 1E FA 41 57 4D 89 C7 41 56 41 55 41 54 49 89 FC 55 53 44 89 CB 48 81 EC 08 01 00 00 48 8B 84 24 40 01 00 00 44 8B AC 24 50 01 00 00 89 74 24 18 48 8D 6C 24 40 44 8B B4 24 48 01 00 00 48 89 54 24 10 48 89 EE 48 89 44 24 28 48 8B 84 24 58 01 00 00 89 4C 24 1C 43 8D 54 0E 0A 44 89 6C 24 04 0F B7 D2 48 89 44 24 20 64 48 8B 04 25 28 00 00 00 48 89 84 24 F8 00 00 00 31 C0 E8 EE 22 04 00


psHkdfExpandLabel(ssl->hsPool,
            hmacAlg,
            inSecret,
            inSecretLen,
            label,
...
*] PRF-Function identified with label: PRF (prf)
[*] PRF-Function signature: undefined prf(void)
[*] String (Register RCX) used as 3th argument in function call at address: 0x00429f32 (invoking PRF)
[*] Function offset (Ghidra): 004348B0 (0x004348B0)
[*] Function offset (IDA with base 0x0): 003348B0 (0x003348B0)
[*] Byte pattern for frida: F3 0F 1E FA 41 57 45 89 CA 41 56 41 55 41 54 55 89 CD 53 89 F3 48 81 EC 48 03 00 00 48 89 7C 24 38 48 89 54 24 08 4C 89 44 24 48 66 44 89 4C 24 50 64 48 8B 04 25 28 00 00 00 48 89 84 24 38 03 00 00 31 C0 66 41 81 F9 E0 00 0F 87 10 02 00 00
