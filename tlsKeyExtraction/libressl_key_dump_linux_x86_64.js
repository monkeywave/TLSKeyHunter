...
function hookLibreSSLByPattern(module) {
                    ...
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                this.session_context = args[5]; // --> ptr to handshake struct
                                this.key = args[0].readPointer(); // when init its zero and only when returning it is filled with the key value
                                //this.key = args[1];
                                this.label = args[3]; // ptr to the label string
                                did_check = false;

                                /*
                                __stdcall tls13_hkdf_expand_label_with_length(tls13_secret * out, EVP_MD * digest, tls13_secret * secret, uint8_t * label, size_t label_len, tls13_secret * context)
                                */

                                /*if(is_arg_key_exp_or_key_hs(args[3])){
                                    this.key = args[0].readPointer();
                                }*/
                                ...
                                //Thread.backtrace(this.context, Backtracer.ACCURATE).map(DebugSymbol.fromAddress).forEach(function (symbol) { console.log("    " + symbol);});


                                /*
                                In order to get the client_Random I need to  hook the tls13_client_engage_record_protection(struct tls13_ctx *ctx)

                                which is internally invoking
                                        ...
                                        try {
                                            for (var i = 0; i < this.myargs.length; i++) {
                                                ...
                                                if (arg) {
                                                    ...
                                                    dumpMemory(arg, 0x580); // Optional: Dump memory at argument address
                                                    console.log("Start checking arguments....");
                                                    //check_arguments(arg, i);
                                                    if(i == 0 || i == 2 || i == 5){
                                                        var region2read = arg.readPointer();
                                                        console.log("REad region: "+i);
                                                        dumpMemory(region2read, 0x580);
                    ...
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                ...
                                /*
                                tls1_PRF(SSL * s, uchar * secret, size_t secret_len, void * seed1, size_t seed1_len, void * seed2, size_t seed2_len, void * seed3, size_t seed3_len, void * seed4, size_t seed4_len, void * seed5, size_t seed5_len, uchar * out, size_t out_len)
                                */
                                

                                if(is_arg_key_exp(args[3])){
                                    this.client_random_ptr = args[7];
                                    this.key = args[1];
                                    this.key_length = args[2].toInt32();
                                    this.dump_keys = true;
                                }
...
// Find the LibreSSL module
function findLibreSSLModule() {
    var modules = Process.enumerateModules();
    for (var i = 0; i < modules.length; i++) {
        var name = modules[i].name;
        if (name.startsWith("libssl.so.58") ) {
            console.log("Found LibreSSL Module: " + name);
            return modules[i];
        }
    }
    console.log("LibreSSL module not found.");
    return null;
}


// test_client_13_libressl_key_export
// Main function
function main() {
    var module = findLibreSSLModule();
    if (module !== null) {
        hookLibreSSLByPattern(module);
    }
}
