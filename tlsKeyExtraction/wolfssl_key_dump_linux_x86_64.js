...
// Function to parse the SSL_HANDSHAKE structure of WolfSSL
...
function check_arguments(memoryPointer, arg_number){
    ...
    try {
        ...
        var memoryContent = dumpMemory(memoryPointer, 0x180);
...
function is_arg_key_exp_or_key_hs(ptr){
    var labelStr = "";
    try{
        if (!ptr.isNull()) {
...
function dump_keys_from_derive_secrets(client_random, key, label_to_span) {
    ...
    // Set the expected length of the key (in bytes). Adjust as needed.
    const MAX_KEY_LENGTH = 64; // Assuming 32 bytes for this example, change this as required.
    var labelStr = "";
    //var client_random = "";
    var secret_key = "";
...
function dump_keys_from_prf(client_random_ptr, key, key_length) {
    ...
    if(!client_random_ptr.isNull()){
        const keyData = Memory.readByteArray(client_random_ptr, 32); // Read the key data (KEY_LENGTH bytes)
...
function get_client_random_from_ssl_connection(args_ptr){
    if (!args_ptr.isNull()) {

        //var clientRandomPtr = ssl_connection.add(0x1C);
        var clientRandomPtr = args_ptr;
        try {
            var clientRandomData = clientRandomPtr.readByteArray(32); // Read 32 bytes
            //console.log("[*] Client Random (offset 0x1C):");
            // Convert the byte array to a string of space-separated hex values
...
function hookWolfSSLByPattern(module) {
    ...
    var pattern_mb_derive_secret_x64 = "F3 0F 1E FA 41 57 45 89 CF 41 56 49 89 CE 41 55 49 89 F5 41 54 49 89 FC 55 4C 89 C5 53 89 D3 48 83 EC 68 64 48 8B 14 25 28 00 00 00 48 89 54 24 58 31 D2 8B 84 24 A0 00 00 00 83 F8 04 74 61";

    
    // Select the appropriate patterns based on the architecture
    var pattern_mb_derive_secret = pattern_mb_derive_secret_x64;
    //console.log("[*] Using HKDF pattern: "+pattern_mb_derive_secret)

    // Try first pattern and if it fails, try the second one
    hook_HKDF_By_Pattern(moduleBase, moduleSize, pattern_mb_derive_secret, "pattern_Tls13DeriveKey");
    var pattern_prf = "F3 0F 1E FA 41 57 41 56 41 55 41 54 55 53 48 81 EC B8 00 00 00 48 89 7C 24 08 4C 8B BC 24 F0 00 00 00 89 74 24 18 8B 9C 24 08 01 00 00 48 89 54 24 10 4C 8B A4 24 10 01 00 00 64 48 8B 04 25 28 00 00 00 48 89 84 24 A8 00 00 00 31 C0 8B 84 24 00 01 00 00 85 C0 0F 84 D4 00 00 00";
    //F3 0F 1E FA FF 25 56 7D 08 00
    hook_PRF_By_Pattern(moduleBase, moduleSize, pattern_prf, "pattern_wc_PRF_TLSt");
}
                    ...
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                ...
                                this.label = args[4]; // ptr to the label string and args[5] is the length of it
                                ...
                                if(is_arg_key_exp_or_key_hs(args[4])){
                                    var arg_str = args[0].add(0x10).readPointer();
                                    this.client_random_ptr = arg_str.add(0x1C);
                                }
                            ...
                            onLeave: function(retval) {
                                        ...
                                        try {
                                            for (var i = 0; i < this.myargs.length; i++) {
                                                ...
                                                if (arg) {
                                                    ...
                                                    dumpMemory(arg, 0x380); // Optional: Dump memory at argument address
                                ...
                                if(session_client_random.length < 2){
                                        session_client_random = get_client_random_from_ssl_connection(this.client_random_ptr);
                                }
                    ...
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                ...
                                /*
                                Argument 2 at address 0x61ec3f8e045d:
                                [*] Argument 3 at address 0x30:
                                [*] Argument 4 at address 0x7af3709ff470:
                                [*] Argument 5 at address 0xf:
                                [*] Argument 6 at address 0x7ffc6761e950:
                                [*] Argument 7 at address 0x30:

                                */
                                

                                if(is_arg_key_exp_or_key_hs(args[4])){
                                    this.client_random_ptr = args[6].add(0x20);
                                    this.key = args[2];
                                    this.key_length = args[3].toInt32();;
                                    this.dump_keys = true;
                                }

                                /*
                                key expansion --> args[2] --> key
                                args3 length

                                label arg4

                                random bei 0ffset 20 bei arg6
                                */
...
// Find the WolfSSL module
function findWolfSSLModule() {
    var modules = Process.enumerateModules();
    for (var i = 0; i < modules.length; i++) {
        var name = modules[i].name;
        if (name.startsWith("libwolfssl") ) {
            console.log("Found WolfSSL Module: " + name);
            return modules[i];
        }
    }
    console.log("WolfSSL module not found.");
    return null;
}
...
// Main function
function main() {
    var module = findWolfSSLModule();
    if (module !== null) {
        hookWolfSSLByPattern(module);
    }
}
...
/*
HKDF: https://github.com/wolfSSL/wolfssl/blob/9bec6da2ff22962f4979e37736d84761c03432b5/wolfcrypt/src/kdf.c#L547

Derive Secret: https://github.com/wolfSSL/wolfssl/blob/9bec6da2ff22962f4979e37736d84761c03432b5/src/dtls13.c#L1948



[*] HKDF-Function identified with label: TLS13DERIVEKEY
[*] String (Register R8) used as 5th argument in function call at address: 0x004130c5 (invoking TLS13DERIVEKEY)
[*] Function offset (Ghidra): 00412A90 (0x00412A90)
[*] Function offset (IDA with base 0x0): 00312A90 (0x00312A90)
[*] Byte pattern for frida (friTap): F3 0F 1E FA 41 57 45 89 CF 41 56 49 89 CE 41 55 49 89 F5 41 54 49 89 FC 55 4C 89 C5 53 89 D3 48 83 EC 68 64 48 8B 14 25 28 00 00 00 48 89 54 24 58 31 D2 8B 84 24 A0 00 00 00 83 F8 04 74 61

PRF https://github.com/wolfSSL/wolfssl/blob/9bec6da2ff22962f4979e37736d84761c03432b5/wolfcrypt/src/kdf.c#L77

[*] PRF-Function identified with label: WC_PRF_TLS
[*] String (Register R8) used as 5th argument in function call at address: 0x0040c9c4 (invoking WC_PRF_TLS)
[*] Function offset (Ghidra): 0041C660 (0x0041C660)
[*] Function offset (IDA with base 0x0): 0031C660 (0x0031C660)
[*] Byte pattern for frida: F3 0F 1E FA 41 57 41 56 41 55 41 54 55 53 48 81 EC B8 00 00 00 48 89 7C 24 08 4C 8B BC 24 F0 00 00 00 89 74 24 18 8B 9C 24 08 01 00 00 48 89 54 24 10 4C 8B A4 24 10 01 00 00 64 48 8B 04 25 28 00 00 00 48 89 84 24 A8 00 00 00 31 C0 8B 84 24 00 01 00 00 85 C0 0F 84 D4 00 00 00
