...
frida -p $(frida-ps | grep -i rust | awk '{print $1}') -l universal_key_dump_liunx_x86_64.js
...
// Function to parse the SSL_HANDSHAKE structure of RusTLS
...
function hookRusTLSByPattern(module) {
...
// Find the RusTLS module
function findRusTLSModule() {
    var modules = Process.enumerateModules();
    for (var i = 0; i < modules.length; i++) {
        var name = modules[i].name;
        if (name.startsWith("test_client_") ) {
            console.log("Found RusTLS Module: " + name);
            return modules[i];
        }
    }
    console.log("RusTLS module not found.");
    return null;
}
...
// Main function
function main() {
    var module = findRusTLSModule();
    if (module !== null) {
        hookRusTLSByPattern(module);
    }
}
