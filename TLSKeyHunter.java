...
/*
 * invocation:
 cd /Users/danielbaier/ghidra/ghidra_11.2_PUBLIC/support
 * than:
 ./analyzeHeadless ~/ghidra_scripts TLSByteScout -import "/Users/danielbaier/research/projects/github/issues/2024 fritap issues/TLSByteScout/binary/libcronet.113.0.5672.61.so" -overwrite -prescript MinimalAnalysisOption.java -postScript TLSKeyHunter.java

 * To-Do:
 *  - Split this into severals files and refactor the code
 *  - Add support for Go, Rust and Java (right now we aren't able to identify there the PRF/HKDF-functions)
 */
...
public class Pair<K, V> {
    ...
    private static final String VERSION = "0.9.1.5";
    ...
    private Address findStringInRodata() throws Exception {
        ...
        Address foundAddress = searchForPattern(memory, start, end, bigEndianPattern);
        ...
        foundAddress = searchForPattern(memory, start, end, littleEndianPattern);
        if (foundAddress != null) {
...
private void extractFunctionInfo(Function function, Address referenceAddress, Boolean is_hkdf, String argument_infos, Address nextInstructionAddr, List<Pair<Function, Address>> list_of_string_refs) {
    ...
    int numBytes = getLengthUntilBranch(function, is_hkdf, do_we_have_two_master_sec_labels);
...
private Pair<Address, Function> getCallTarget(Instruction instruction) {
    ...
    if(instruction.getReferencesFrom().length > 0){
        Reference ref = instruction.getReferencesFrom()[0]; // we assume that the first reference is the one we are looking for and it is better than nothing
        ...
        for (PcodeOp pcodeOp : pcodeOps) {
            ...
            for (int i = 0; i < pcodeOp.getNumInputs(); i++) {
                ...
                if (input.equals(trackedVarnode) || pcodeOp.getOpcode() == PcodeOp.CALL || pcodeOp.getOpcode() == PcodeOp.CALLIND) { 
                    ...
                    if ((pcodeOp.getOpcode() == PcodeOp.CALL || pcodeOp.getOpcode() == PcodeOp.CALLIND) && (can_we_use_next_call || do_rerun)) {
                        ...
                        String argument_infos = "[*] String (Register "+result_register_name+") used as " + (argumentIndex+1);
            ...
            if(pcodeOp.getOpcode() == PcodeOp.STORE && pcodeOp.getOpcode() != PcodeOp.CALL && !currentInstruction.toString().toUpperCase().startsWith("CALL")){
                ...
                if (previous_pcode_was_copy) {
                    ...
                    Register oldRegister = getRegisterForVarnode(trackedVarnode);
...
private Address findFunctionCallUsingResult(Address startAddress) {
    ...
    private int getLengthUntilBranch(Function function, boolean is_hkdf, boolean has_more_than_one_master_sec) {
        ...
        while (true) {
            ...
            if (currentInstruction.getFlowType().isCall()) {
                ...
                Address calledFunctionAddr = currentInstruction.getFlows()[0];
                ...
                if (calledFunction != null && calledFunction.getName().contains("strlen")) {
                    if(DEBUG_RUN){
                ...
                }else if(calledFunction != null && calledFunction.getName().contains("_ZN4bssl4SpanIKhEC1EPS1_m") || calledFunction.getName().contains("Span")){
                    if(DEBUG_RUN){
                ...
                }else if(calledFunction != null && calledFunction.getName().contains("_ZNK4bssl8internal21StackAllocatedMovableI13en") || calledFunction.getName().contains("GET") || calledFunction.getName().contains("_M_ptr")){
                    if(DEBUG_RUN){
            ...
            if (currentInstruction.getFlowType().isJump() ||
                currentInstruction.getFlowType().isConditional()) {
                    ...
                    if(currentProgram.getLanguageID().toString().contains("32")){
    ...
    protected void run() throws Exception {
        ...
        System.out.println("[*] Thx for using TLSKeyHunter. Have a nice day :)");
        System.out.println();
        

        /*
         * We try to identify the derive_secret() function which is in TLS 1.3 the HKDF
         * In some libraries (e.g. BoringSSL) the span the buffer before this buffer is provided as an argument.
         * The span function usually have a strlen() invocation and is quite short
         * The key is usually stored in some provided buffer of the final identified function
         * 
         * Bytes label_to_span
         3F 23 03 D5 FD 7B BE A9  F3 0B 00 F9 FD 03 00 91
         F3 03 00 AA 33 31 04 94  E1 03 00 AA E0 03 13 AA
         F3 0B 40 F9 FD 7B C2 A8  BF 23 03 D5 C0 03 5F D6

         3F 23 03 D5 FD 7B BE A9 F3 0B 00 F9 FD 03 00 91 F3 03 00 AA 33 31 04 94
         3F 23 03 D5 FD 7B BE A9 F3 0B 00 F9 FD 03 00 91 F3 03 00 AA 33 31 04 94

         * Bytes derive_secret
         3F 23 03 D5 FF C3 02 D1  FD 7B 06 A9 FA 67 07 A9
F8 5F 08 A9 F6 57 09 A9  F4 4F 0A A9 FD 83 01 91
59 D0 3B D5 F5 03 02 AA  F6 03 01 AA 28 17 40 F9
F7 03 00 AA E1 63 00 91  E2 43 00 91 E0 03 03 AA
F3 03 05 AA F4 03 04 AA  F8 03 03 AA A8 83 1F F8

3F 23 03 D5 FF C3 02 D1 FD 7B 06 A9 FA 67 07 A9 F8 5F 08 A9 F6 57 09 A9 F4 4F 0A A9 FD 83 01 91 59 D0 3B D5 F5 03 02 AA F6 03 01 AA 28 17 40 F9 F7 03 00 AA E1 63 00 91 E2 43 00 91 E0 03 03 AA F3 03 05 AA F4 03 04 AA F8 03 03 AA A8 83 1F F8
3F 23 03 D5 FF C3 02 D1 FD 7B 06 A9 FA 67 07 A9 F8 5F 08 A9 F6 57 09 A9 F4 4F 0A A9 FD 83 01 91 59 D0 3B D5 F5 03 02 AA F6 03 01 AA 28 17 40 F9 F7 03 00 AA E1 63 00 91 E2 43 00 91 E0 03 03 AA F3 03 05 AA F4 03 04 AA F8 03 03 AA A8 83 1F F8 FA 63 00 91 FE ED FF 97
         * 
         */
    }
}
