...
frida -p $(frida-ps | grep -i lighttpd | awk '{print $1}') -l mbedtls_key_dump_linux_x86_64.js
...
var invoke_fallback_key_dump = true;
...
// Function to parse the SSL_HANDSHAKE structure of MBedTLS
...
function hookMBedTLSByPattern(module) {
    ...
    var pattern_mb_derive_secret = "F3 0F 1E FA 55 48 89 E5 48 81 EC A0 00 00 00 89 7D 9C 48 89 75 90 48 89 55 88 48 89 4D 80 4C 89 85 78 FF FF FF 4C 89 8D 70 FF FF FF 48 8B 45 20 48 89 85 68 FF FF FF 64 48 8B 04 25 28 00 00 00 48 89 45 F8 31 C0 83 7D 18 00 0F 85 AA 01 00 00";
    //console.log("[*] Using HKDF pattern: "+pattern_mb_derive_secret)

    // Try first pattern and if it fails, try the second one
    hook_HKDF_By_Pattern(moduleBase, moduleSize, pattern_mb_derive_secret, "mbedtls_ssl_tls13_derive_secret");
    //var pattern_prf = "F3 0F 1E FA 55 B9 85 06 00 00 BE 02 00 00 00 4C 8D 05 EA 7A 00 00 48 89 E5 41 57 41 56 4C 8D 35 EA 78 00 00 41 55 4C 89 F2 41 54 53 48 89 FB 48 83 EC 68 64 48 8B 04 25 28 00 00 00 48 89 45 C8 48 8B 47 70 4C 8B 60 18 31 C0 E8 C1 A1 FE FF";
    var pattern_prf = "55 48 89 E5 41 57 41 56 41 89 F6 41 55 41 54 53 48 89 FB 48 81 EC 68 01 00 00 48 8B 45 28 48 89 95 B0 FE FF FF 89 8D 88 FE FF FF 4C 8B 65 10 44 89 85 90 FE FF FF 4C 89 8D A0 FE FF FF 48 89 85 B8 FE FF FF 64 48 8B 04 25 28 00 00 00 48 89 45";

    hook_PRF_By_Pattern(moduleBase, moduleSize, pattern_prf, "ssl_compute_master");
}
                    ...
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                //console.log("[!] __int64 __fastcall mbedtls_ssl_tls13_derive_secret invoked...")

                                // __int64 __fastcall mbedtls_ssl_tls13_derive_secret(int, int, int, int, int, int, size_t n, int, __int64, __int64)
                                /*
                                nt mbedtls_ssl_tls13_derive_secret(
                                ...
                                */
                                ...
                                this.key = args[8]; // when init its zero and only when returning it is filled with the key value
                                this.key_len = args[9].toInt32();
                                this.label = args[3]; // ptr to the label string
                            ...
                            onLeave: function(retval) {
                                ...
                                // currently we don't get the CLIENT_RANDOM as there is no context in the argument list
...
function hook_PRF_By_Pattern(moduleBase, moduleSize, pattern, pattern_name){
    ...
    Memory.scan(moduleBase, moduleSize, pattern, {
                    onMatch: function(address, size) {
                        ...
                        Interceptor.attach(address, {
                            onEnter: function(args) {
                                this.fallback_key = args[1]; // currently not used
                              ...
                              const mbedtls_ssl_context *ssl)
                                */
                                ...
                                // arg1 is also containing the calculated master secret 

                                try{
                                    did_check = true;

                                    /*
                                    mov     [rbp+ptr_to_prf], rdi
                                    ...
                                    mov     rax, [rbp+ptr_to_prf] // this is the same like saving var rax = args[0];
                                    mov     r10, [rax+30h] // then we have to add 0x30 to the final value and than dereference --> rax.add(0x30).readPointer();
                                    mov     rsi, [rbp+var_60]
                                    mov     rax, [rbp+ptr_to_prf]
                                    mov     rax, [rax+0CC0h]
                                    mov     rdx, [rbp+ptr_to_prf]
                                    lea     rdi, [rdx+888h]
                                    mov     r8, [rbp+var_80]
                                    mov     rcx, [rbp+var_50]
                                    mov     rdx, [rbp+str_master_secret]
                                    sub     rsp, 8
                                    push    [rbp+var_48]
                                    mov     r9, r8
                                    mov     r8, rsi
                                    mov     rsi, rax
                                    call    r10  // the final function invocation
                                    */
                                        
                                        // probably the handshake->tls_prf() function
                                        var rax = args[0]; // mov [rbp+ptr2prf], RDI
                                        var final_rax = rax.add(0x30).readPointer();
                                        Interceptor.attach(final_rax, { 
                                            onEnter: function (in_args){
                                                //console.log("[!] Hooked handshake->tls_prf()");
                                                ...
                                                */
                                                ...
                                                this.label = in_args[2];
                                                this.key = in_args[0];
                                                this.key_len = in_args[1].toInt32();
                                                this.client_random_ptr = in_args[3];

              
                                                if(is_arg_key_exp(this.label)){
                                                    dump_keys_from_prf(this.client_random_ptr, this.key, this.key_len);
                                                    invoke_fallback_key_dump = false;
                                                }
                                            ...
                                            onLeave: function (retval){
                                                if(invoke_fallback_key_dump){
                                                    //dump_keys_from_prf(0, this.fallback_key, 32);
                                                }
...
// Find the MBedTLS module
function findMBedTLSModule() {
    var modules = Process.enumerateModules();
    for (var i = 0; i < modules.length; i++) {
        var name = modules[i].name;
        if (name.startsWith("libmbedtls") ) {
            console.log("Found MBedTLS Module: " + name);
            return modules[i];
        }
    }
    console.log("MBedTLS module not found.");
    return null;
}
...
// Main function
function main() {
    var module = findMBedTLSModule();
    if (module !== null) {
        hookMBedTLSByPattern(module);
    }
}
...
/*
[*] HKDF-Function identified with label: MBEDTLS_SSL_TLS13_DERIVE_SECRET (mbedtls_ssl_tls13_derive_secret)
[*] HKDF-Function signature: undefined mbedtls_ssl_tls13_derive_secret(undefined param_1, undefined param_2, undefined param_3, undefined param_4, undefined param_5, undefined param_6, undefined8 param_7, undefined4 param_8, undefined8 param_9, undefined8 param_10)
[*] String (Register RCX) used as 3th argument in function call at address: 0x0013b79d (invoking MBEDTLS_SSL_TLS13_DERIVE_SECRET)
[*] Function offset (Ghidra): 0013AAAB (0x0013AAAB)
[*] Function offset (IDA with base 0x0): 0003AAAB (0x0003AAAB)
[*] Byte pattern for frida: F3 0F 1E FA 55 48 89 E5 48 81 EC A0 00 00 00 89 7D 9C 48 89 75 90 48 89 55 88 48 89 4D 80 4C 89 85 78 FF FF FF 4C 89 8D 70 FF FF FF 48 8B 45 20 48 89 85 68 FF FF FF 64 48 8B 04 25 28 00 00 00 48 89 45 F8 31 C0 83 7D 18 00 0F 85 AA 01 00 00


https://github.com/Mbed-TLS/mbedtls/blob/9c2afb44bb7509a135ef9a8b6349b5617e8241d1/library/ssl_tls13_keys.c#L293
...
[*] To assist, we provide the offset of the indirect call, the argument index of the label for the PRF-function being used, and byte patterns of the calling function for Frida hooking.
Function: ssl_compute_master

[*] PRF-Function identified as indirect call: CALL R10
[*] Function offset (Ghidra): 001268CB (0x001268CB)
[*] Function offset (IDA with base 0x0): 000268CB (0x000268CB)


[*] PRF-Function identified with label: SSL_COMPUTE_MASTER (ssl_compute_master)
[*] PRF-Function signature: undefined ssl_compute_master(void)
[*] Function offset (Ghidra): 0012673B (0x0012673B)
[*] Function offset (IDA with base 0x0): 0002673B (0x0002673B)
[*] Byte pattern for frida: F3 0F 1E FA 55 48 89 E5 48 81 EC 90 00 00 00 48 89 7D 88 48 89 75 80 48 89 95 78 FF FF FF 64 48 8B 04 25 28 00 00 00 48 89 45 F8 31 C0 C7 45 9C 92 FF FF FF 48 C7 45 B8 30 00 00 00 48 8D 05 9B 04 03 00 48 89 45 A8 48 8B 45 88 48 05 48 08 00 00 48 89 45 B0 48 C7 45 A0 40 00 00 00 48 8B 45 88 0F B6 00 84 C0 74 36


https://github.com/Mbed-TLS/mbedtls/blob/9c2afb44bb7509a135ef9a8b6349b5617e8241d1/library/ssl_tls.c#L6437

tls_prf(secret, slen, label, random, rlen, dstbuf, dlen);
handshake->tls_prf(handshake->premaster, handshake->pmslen,
                                 lbl, seed, seed_len,
                                 master,
                                 master_secret_len);
