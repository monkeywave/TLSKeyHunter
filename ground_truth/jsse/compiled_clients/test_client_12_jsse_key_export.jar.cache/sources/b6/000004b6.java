...
/* loaded from: test_client_12_jsse_key_export.jar:org/bouncycastle/crypto/generators/DSAParametersGenerator.class */
public class DSAParametersGenerator {
    ...
    public void init(int i, int i2, SecureRandom secureRandom) {
        this.f401L = i;
        this.f402N = getDefaultN(i);
        this.certainty = i2;
        this.iterations = Math.max(getMinimumIterations(this.f401L), (i2 + 1) / 2);
    ...
    private DSAParameters generateParameters_FIPS186_2() {
        ...
        int i = (this.f401L - 1) / Opcode.IF_ICMPNE;
        byte[] bArr5 = new byte[this.f401L / 8];
            ...
            if (isProbablePrime(bigInteger)) {
                ...
                for (int i3 = 0; i3 < 4096; i3++) {
                    for (int i4 = 1; i4 <= i; i4++) {
                        inc(clone);
                        hash(this.digest, clone, bArr5, bArr5.length - (i4 * bArr2.length));
                    }
                    int length = bArr5.length - (i * bArr2.length);
                    inc(clone);
                    ...
                    if (subtract.bitLength() == this.f401L && isProbablePrime(subtract)) {
    ...
    private DSAParameters generateParameters_FIPS186_3() {
        ...
        byte[] bArr = new byte[this.f402N / 8];
        int i2 = (this.f401L - 1) / digestSize;
        int i3 = (this.f401L - 1) % digestSize;
        byte[] bArr2 = new byte[this.f401L / 8];
        byte[] bArr3 = new byte[digest.getDigestSize()];
        loop0: while (true) {
            this.random.nextBytes(bArr);
            hash(digest, bArr, bArr3, 0);
            bit = new BigInteger(1, bArr3).mod(ONE.shiftLeft(this.f402N - 1)).setBit(0).setBit(this.f402N - 1);
            if (isProbablePrime(bit)) {
                ...
                while (i < i4) {
                    for (int i5 = 1; i5 <= i2; i5++) {
                        inc(clone);
                        hash(digest, clone, bArr2, bArr2.length - (i5 * bArr3.length));
                    }
