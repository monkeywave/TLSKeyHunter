...
/* loaded from: test_client_12_jsse_key_export.jar:org/bouncycastle/math/ec/LongArray.class */
public class LongArray implements Cloneable {
    ...
    public LongArray modMultiplyAlt(LongArray longArray, int i, int[] iArr) {
        ...
        int i5 = ((degree2 + (64 < 64 ? 16 : 16 - 1)) + 63) >>> 6;
        int i6 = i5 * 8;
        int i7 = 4 * 8;
    ...
    private static int reduceInPlace(long[] jArr, int i, int i2, int i3, int[] iArr) {
        ...
        int max = Math.max(i3, i6 + 64);
        int min2 = (i5 + Math.min(min - max, i3 - i7)) >> 6;
        if (min2 > 1) {
            int i8 = i2 - min2;
        ...
        if (min > max) {
            reduceWordWise(jArr, i, i2, max, i3, iArr);
            min = max;
        }
        if (min > i3) {
            reduceBitWise(jArr, i, min, i3, iArr);
        }
        return i4;
    }
    ...
    private static long interleave3_21to63(int i) {
        int i2 = INTERLEAVE3_TABLE[i & Opcode.LAND];
        return ((INTERLEAVE3_TABLE[i >>> 14] & 4294967295L) << 42) | ((INTERLEAVE3_TABLE[(i >>> 7) & Opcode.LAND] & 4294967295L) << 21) | (i2 & 4294967295L);
    }
    ...
    private static long interleave3_13to65(int i) {
        return ((INTERLEAVE5_TABLE[i >>> 7] & 4294967295L) << 35) | (INTERLEAVE5_TABLE[i & Opcode.LAND] & 4294967295L);
    }
    ...
    private static long interleave2_n(long j, int i) {
        while (i > 1) {
            i -= 2;
            j = interleave4_16to64(((int) j) & Blake2xsDigest.UNKNOWN_DIGEST_LENGTH) | (interleave4_16to64(((int) (j >>> 16)) & Blake2xsDigest.UNKNOWN_DIGEST_LENGTH) << 1) | (interleave4_16to64(((int) (j >>> 32)) & Blake2xsDigest.UNKNOWN_DIGEST_LENGTH) << 2) | (interleave4_16to64(((int) (j >>> 48)) & Blake2xsDigest.UNKNOWN_DIGEST_LENGTH) << 3);
        }
        if (i > 0) {
    ...
    private static long interleave4_16to64(int i) {
        return ((INTERLEAVE4_TABLE[i >>> 8] & 4294967295L) << 32) | (INTERLEAVE4_TABLE[i & GF2Field.MASK] & 4294967295L);
    }

    private static long interleave2_32to64(int i) {
        return (((INTERLEAVE2_TABLE[(i >>> 16) & GF2Field.MASK] | (INTERLEAVE2_TABLE[i >>> 24] << 16)) & 4294967295L) << 32) | ((INTERLEAVE2_TABLE[i & GF2Field.MASK] | (INTERLEAVE2_TABLE[(i >>> 8) & GF2Field.MASK] << 16)) & 4294967295L);
    }
    ...
    public LongArray modInverse(int i, int[] iArr) {
        ...
        while (true) {
            ...
            longArrayArr[i3].addShiftedByBitsSafe(longArrayArr[1 - i3], iArr2[1 - i3], i6);
            int degreeFrom = longArrayArr[i3].degreeFrom(i4);
            if (degreeFrom == 0) {
                return longArrayArr2[1 - i3];
            }
            int i7 = iArr3[1 - i3];
            longArrayArr2[i3].addShiftedByBitsSafe(longArrayArr2[1 - i3], i7, i6);
            int i8 = i7 + i6;
            if (i8 > i5) {
                i5 = i8;
            } else if (i8 == i5) {
