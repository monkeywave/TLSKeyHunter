...
/* loaded from: test_client_12_jsse_key_export.jar:org/bouncycastle/pqc/crypto/lms/LM_OTS.class */
public class LM_OTS {
    ...
    public static int coef(byte[] bArr, int i, int i2) {
        return (bArr[(i * i2) / 8] >>> (i2 * ((i ^ (-1)) & ((8 / i2) - 1)))) & ((1 << i2) - 1);
    }
    ...
    public static byte[] lms_ots_generatePublicKey(LMOtsParameters lMOtsParameters, byte[] bArr, int i, byte[] bArr2) {
        ...
        byte[] build = Composer.compose().bytes(bArr).u32str(i).u16str(D_PBLC).padUntil(0, 22).build();
        digest.update(build, 0, build.length);
        Digest digest2 = DigestUtil.getDigest(lMOtsParameters.getDigestOID());
        byte[] build2 = Composer.compose().bytes(bArr).u32str(i).padUntil(0, 23 + digest2.getDigestSize()).build();
    ...
    public static LMOtsSignature lm_ots_generate_signature(LMOtsPrivateKey lMOtsPrivateKey, byte[] bArr, byte[] bArr2) {
        ...
        bArr[n + 1] = (byte) cksm;
        byte[] build = Composer.compose().bytes(lMOtsPrivateKey.getI()).u32str(lMOtsPrivateKey.getQ()).padUntil(0, 23 + n).build();
        derivationFunction.setJ(0);
        int i = 0;
        while (i < p) {
            Pack.shortToBigEndian((short) i, build, 20);
            derivationFunction.deriveSeed(build, i < p - 1, 23);
            int coef = coef(bArr, i, w);
            for (int i2 = 0; i2 < coef; i2++) {
                build[22] = (byte) i2;
                digest.update(build, 0, 23 + n);
                digest.doFinal(build, 23);
            }
            System.arraycopy(build, 23, bArr3, n * i, n);
            i++;
        }
    ...
    public static byte[] lm_ots_validate_signature_calculate(LMSContext lMSContext) {
        ...
        byte[] build = Composer.compose().bytes(i).u32str(q2).padUntil(0, 23 + n).build();
        int i2 = (1 << w) - 1;
        ...
        for (int i3 = 0; i3 < p; i3++) {
            ...
            System.arraycopy(y, i3 * n, build, 23, n);
            for (int coef = coef(q, i3, w); coef < i2; coef++) {
                build[22] = (byte) coef;
