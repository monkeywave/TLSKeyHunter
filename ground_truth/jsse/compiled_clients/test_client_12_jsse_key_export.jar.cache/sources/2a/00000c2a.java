...
/* loaded from: test_client_12_jsse_key_export.jar:org/bouncycastle/math/ec/ECAlgorithms.class */
public class ECAlgorithms {
    ...
    public static ECPoint sumOfTwoMultiplies(ECPoint eCPoint, BigInteger bigInteger, ECPoint eCPoint2, BigInteger bigInteger2) {
        ...
        ECEndomorphism endomorphism = curve.getEndomorphism();
        return endomorphism instanceof GLVEndomorphism ? implCheckResult(implSumOfMultipliesGLV(new ECPoint[]{eCPoint, importPoint}, new BigInteger[]{bigInteger, bigInteger2}, (GLVEndomorphism) endomorphism)) : implCheckResult(implShamirsTrickWNaf(eCPoint, bigInteger, importPoint, bigInteger2));
    }
    ...
    public static void montgomeryTrick(ECFieldElement[] eCFieldElementArr, int i, int i2, ECFieldElement eCFieldElement) {
        ...
        while (true) {
            ECFieldElement eCFieldElement2 = invert;
            if (i4 <= 0) {
                eCFieldElementArr[i] = eCFieldElement2;
                return;
            }
            int i5 = i4;
            i4--;
            int i6 = i + i5;
            ECFieldElement eCFieldElement3 = eCFieldElementArr[i6];
            eCFieldElementArr[i6] = eCFieldElementArr2[i4].multiply(eCFieldElement2);
            invert = eCFieldElement2.multiply(eCFieldElement3);
        }
    }
    ...
    static ECPoint implShamirsTrickJsf(ECPoint eCPoint, BigInteger bigInteger, ECPoint eCPoint2, BigInteger bigInteger2) {
        ...
        while (true) {
            ...
            eCPoint3 = eCPoint3.twicePlus(eCPointArr2[4 + (((b << 24) >> 28) * 3) + ((b << 28) >> 28)]);
        }
    }
    ...
    private static ECPoint implShamirsTrickWNaf(ECPoint[] eCPointArr, ECPoint[] eCPointArr2, byte[] bArr, ECPoint[] eCPointArr3, ECPoint[] eCPointArr4, byte[] bArr2) {
        ...
        while (i2 >= 0) {
            ...
            if ((b | b2) == 0) {
                ...
                if (b != 0) {
                    eCPoint2 = eCPoint2.add((b < 0 ? eCPointArr2 : eCPointArr)[Math.abs((int) b) >>> 1]);
                }
                if (b2 != 0) {
                    eCPoint2 = eCPoint2.add((b2 < 0 ? eCPointArr4 : eCPointArr3)[Math.abs((int) b2) >>> 1]);
                }
                if (i > 0) {
    ...
    static ECPoint implSumOfMultiplies(ECPoint[] eCPointArr, BigInteger[] bigIntegerArr) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            r0[i] = WNafUtil.generateWindowNaf(min, abs);
        }
        return implSumOfMultiplies(zArr, wNafPreCompInfoArr, (byte[][]) r0);
    }
    ...
    static ECPoint implSumOfMultiplies(ECEndomorphism eCEndomorphism, ECPoint[] eCPointArr, BigInteger[] bigIntegerArr) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            ...
            r0[i3] = WNafUtil.generateWindowNaf(min, abs);
            r0[i4] = WNafUtil.generateWindowNaf(min2, abs2);
        }
        return implSumOfMultiplies(zArr, wNafPreCompInfoArr, (byte[][]) r0);
    }
    ...
    private static ECPoint implShamirsTrickFixedPoint(ECPoint eCPoint, BigInteger bigInteger, ECPoint eCPoint2, BigInteger bigInteger2) {
        ...
        for (int i4 = 0; i4 < i; i4++) {
            ...
            while (true) {
                int i9 = i7 - i8;
                if (i9 >= 0) {
                    int i10 = fromBigInteger[i9 >>> 5] >>> (i9 & 31);
                    i5 = ((i5 ^ (i10 >>> 1)) << 1) ^ i10;
                    int i11 = fromBigInteger2[i9 >>> 5] >>> (i9 & 31);
                    i6 = ((i6 ^ (i11 >>> 1)) << 1) ^ i11;
                    i7 = i9;
                    i8 = i;
                }
