...
class Fors {
    ...
    public byte[] pkFromSig(SIG_FORS[] sig_forsArr, byte[] bArr, byte[] bArr2, ADRS adrs) {
        ...
        byte[][] bArr4 = new byte[this.engine.f1428K];
        int i2 = this.engine.f1430T;
        int[] message_to_idxs = message_to_idxs(bArr, this.engine.f1428K, this.engine.f1425A);
        int i3 = 0;
        while (i3 < this.engine.f1428K) {
            int i4 = message_to_idxs[i3];
            byte[] sk = sig_forsArr[i3].getSK();
            adrs.setTreeHeight(0);
            int i5 = (i3 * i2) + i4;
            adrs.setTreeIndex(i5);
            bArr3[0] = this.engine.mo13F(bArr2, adrs, sk);
            byte[][] authPath = sig_forsArr[i3].getAuthPath();
            adrs.setTreeIndex(i5);
            int i6 = 0;
            while (i6 < this.engine.f1425A) {
                int i7 = i6 + 1;
                adrs.setTreeHeight(i7);
                if ((i4 / (1 << i6)) % i == 0) {
                    adrs.setTreeIndex(adrs.getTreeIndex() / i);
                    bArr3[1] = this.engine.mo12H(bArr2, adrs, bArr3[0], authPath[i6]);
                } else {
                    adrs.setTreeIndex((adrs.getTreeIndex() - 1) / 2);
                    bArr3[1] = this.engine.mo12H(bArr2, adrs, authPath[i6], bArr3[0]);
                }
                bArr3[0] = bArr3[1];
                i6 = i7;
                i = 2;
    ...
    public SIG_FORS[] sign(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        Fors fors = this;
        ADRS adrs2 = new ADRS(adrs);
        int[] message_to_idxs = message_to_idxs(bArr, fors.engine.f1428K, fors.engine.f1425A);
        SIG_FORS[] sig_forsArr = new SIG_FORS[fors.engine.f1428K];
        int i = fors.engine.f1430T;
        int i2 = 0;
        int i3 = 0;
        while (i3 < fors.engine.f1428K) {
            int i4 = message_to_idxs[i3];
            ...
            adrs2.changeType(3);
            byte[][] bArr4 = new byte[fors.engine.f1425A];
            int i6 = i2;
            while (i6 < fors.engine.f1425A) {
                int i7 = 1 << i6;
                int i8 = i6;
                byte[][] bArr5 = bArr4;
    ...
    byte[] treehash(byte[] bArr, int i, int i2, byte[] bArr2, ADRS adrs) {
        ...
        for (int i3 = 0; i3 < (1 << i2); i3++) {
            ...
            adrs2.changeType(3);
            byte[] mo13F = this.engine.mo13F(bArr2, adrs2, PRF);
            adrs2.setTreeHeight(1);
            int i5 = 1;
            while (!linkedList.isEmpty() && ((NodeEntry) linkedList.get(0)).nodeHeight == i5) {
                i4 = (i4 - 1) / 2;
                adrs2.setTreeIndex(i4);
                mo13F = this.engine.mo12H(bArr2, adrs2, ((NodeEntry) linkedList.remove(0)).nodeValue, mo13F);
                i5++;
                adrs2.setTreeHeight(i5);
            }
            linkedList.add(0, new NodeEntry(mo13F, i5));
        }
