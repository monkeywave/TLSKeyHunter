...
import com.example.tls_library_ground_truth.BuildConfig;
...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedList.CLOSED) {
                        m1892constructorimpl = SegmentOrClosed.m1892constructorimpl(ConcurrentLinkedList.CLOSED);
                        break;
                    }
                    ...
                    m1892constructorimpl = SegmentOrClosed.m1892constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1897isClosedimpl(m1892constructorimpl)) {
                Segment m1895getSegmentimpl = SegmentOrClosed.m1895getSegmentimpl(m1892constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m1895getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m1895getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (C0135xc40028dd.m296m(tail$FU, this, segment, m1895getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m1895getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1895getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1895getSegmentimpl(m1892constructorimpl);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (andIncrement % i2);
        if (!ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m149m(semaphoreSegment4.acquirers, i3, null, cancellableContinuation)) {
            symbol = SemaphoreKt.PERMIT;
            symbol2 = SemaphoreKt.TAKEN;
            if (ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m149m(semaphoreSegment4.acquirers, i3, symbol, symbol2)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m1895getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (C0135xc40028dd.m296m(head$FU, this, segment, m1895getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1895getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
        ...
        if (andSet == null) {
            ...
            return !ConcurrentWeakMap$Core$$ExternalSyntheticBackportWithForwarding0.m149m(semaphoreSegment4.acquirers, i4, symbol3, symbol4);
        }
