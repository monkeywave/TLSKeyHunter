...
class WotsPlus {
    ...
    byte[] chain(byte[] bArr, int i, int i2, byte[] bArr2, ADRS adrs) {
        ...
        if (i + i2 > this.f1422w - 1) {
            return null;
        }
        for (int i3 = 0; i3 < i2; i3++) {
            adrs.setHashAddress(i + i3);
            bArr = this.engine.mo15F(bArr2, adrs, bArr);
        }
        return bArr;
    }
    ...
    public byte[] pkFromSig(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        ...
        base_w(bArr2, 0, this.f1422w, iArr, 0, this.engine.WOTS_LEN1);
        int i = 0;
        for (int i2 = 0; i2 < this.engine.WOTS_LEN1; i2++) {
            i += (this.f1422w - 1) - iArr[i2];
        }
        base_w(Pack.intToBigEndian(i << (8 - ((this.engine.WOTS_LEN2 * this.engine.WOTS_LOGW) % 8))), 4 - (((this.engine.WOTS_LEN2 * this.engine.WOTS_LOGW) + 7) / 8), this.f1422w, iArr, this.engine.WOTS_LEN1, this.engine.WOTS_LEN2);
        byte[] bArr4 = new byte[this.engine.f1404N];
        byte[][] bArr5 = new byte[this.engine.WOTS_LEN];
        for (int i3 = 0; i3 < this.engine.WOTS_LEN; i3++) {
            adrs.setChainAddress(i3);
            System.arraycopy(bArr, this.engine.f1404N * i3, bArr4, 0, this.engine.f1404N);
            int i4 = iArr[i3];
            bArr5[i3] = chain(bArr4, i4, (this.f1422w - 1) - i4, bArr3, adrs);
        }
    ...
    public byte[] pkGen(byte[] bArr, byte[] bArr2, ADRS adrs) {
        ...
        for (int i = 0; i < this.engine.WOTS_LEN; i++) {
            ...
            adrs3.setChainAddress(i);
            adrs3.setHashAddress(0);
            bArr3[i] = chain(PRF, 0, this.f1422w - 1, bArr2, adrs3);
        }
        adrs2.setTypeAndClear(1);
        adrs2.setKeyPairAddress(adrs.getKeyPairAddress());
    ...
    public byte[] sign(byte[] bArr, byte[] bArr2, byte[] bArr3, ADRS adrs) {
        ADRS adrs2 = new ADRS(adrs);
        int[] iArr = new int[this.engine.WOTS_LEN];
        base_w(bArr, 0, this.f1422w, iArr, 0, this.engine.WOTS_LEN1);
        int i = 0;
        for (int i2 = 0; i2 < this.engine.WOTS_LEN1; i2++) {
            i += (this.f1422w - 1) - iArr[i2];
        }
        ...
        base_w(Pack.intToBigEndian(i), 4 - (((this.engine.WOTS_LEN2 * this.engine.WOTS_LOGW) + 7) / 8), this.f1422w, iArr, this.engine.WOTS_LEN1, this.engine.WOTS_LEN2);
