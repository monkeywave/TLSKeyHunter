...
void printExportedKeys(){
    FILE* keylog_file = fopen("keylog.txt", "r");
    if (keylog_file == NULL) {
        printf("Unable to open keylog.txt\n");
        return;
    }

    char ch;
    while ((ch = fgetc(keylog_file)) != EOF) {
        putchar(ch);
    }
...
int main(){
    ...
    // Remove keylog.txt if it exists and create a new one
    remove("keylog.txt");
    FILE* keylog_file = fopen("keylog.txt", "w");
    if (keylog_file == NULL) {
        printf("Unable to create keylog.txt\n");
        return 1;
    }
    fclose(keylog_file);
    
    // set environment variable for key export
    setenv("SSLKEYLOGFILE", "keylog.txt", 1);
    ...
    // Export keying material and print it
    printExportedKeys();
