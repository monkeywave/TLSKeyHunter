...
void key_log_callback(struct rustls_str label, const uint8_t *client_random, size_t client_random_len,
    const uint8_t *secret, size_t secret_len) {
    printf("KeyLogcallback \n");
    // Print the label
    printf("%.*s ", (int)label.len, label.data);

    // Print the client random as a hex string
    for (size_t i = 0; i < client_random_len; i++) {
        printf("%02x", client_random[i]);
    }
    printf(" ");

    // Print the secret as a hex string
...
int main() {
    ...
    // Set the KeyLogCB
    if (rustls_client_config_builder_set_key_log(config_builder, (rustls_keylog_log_callback)key_log_callback, NULL) != RUSTLS_RESULT_OK) {
        fprintf(stderr, "Error setting KeyLogCB \n");
        return -1;
    } 
